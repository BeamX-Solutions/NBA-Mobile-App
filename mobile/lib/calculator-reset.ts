 ...
 * calculator invites the practitioner to generate a second invoice for the
 ...
 * mounted while the invoice flow runs above it, so there is no re-mount to
 * hang a reset on, and the calculator has no other reason to know that the
 * invoice screen exists.
 *
