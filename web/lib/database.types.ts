 ...
 *
 * Copied from mobile/lib/database.types.ts. The console does not depend on the
 * fee engine — it never calculates a fee, it displays figures a practitioner
 * already committed to — so the engine's DocumentType import is replaced here
 * by the document_type enum spelled out directly. Both files are derived from
 * the same migration and should be replaced by `supabase gen types` output.
 */
type EngineDocumentType =
  | 'deed_of_assignment'
  | 'deed_of_conveyance'
  | 'deed_of_gift'
  | 'contract_of_sale'
